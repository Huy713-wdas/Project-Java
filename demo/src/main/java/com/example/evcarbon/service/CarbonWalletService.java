...
// LẤY HOẶC TẠO MỚI VÍ TÍN CHỈ CHO CHỦ SỞ HƯU
    public CarbonWallet getOrCreate(Long ownerId) {
        ...
        return opt.orElseGet(() -> walletRepository.save(
                CarbonWallet.builder()
                        .ownerId(ownerId)
                        .balance(0.0)
                        .cashBalance(0.0)
                        .build()));
...
return true;
    ...
    public void addCash(Long ownerId, double amount) {
        CarbonWallet w = getOrCreate(ownerId);
        w.setCashBalance(w.getCashBalance() + amount);
        walletRepository.save(w);
    }

    public boolean withdrawCash(Long ownerId, double amount) {
        CarbonWallet w = getOrCreate(ownerId);
        if (w.getCashBalance() < amount) return false;
        w.setCashBalance(w.getCashBalance() - amount);
        walletRepository.save(w);
        return true;
    }
